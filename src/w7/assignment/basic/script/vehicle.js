class Vehicle {
  ...
  constructor(x, y, mass, rad, speedMx, forceMx, color) {
    //생성된 개채의 특성에 대한 초기값을 설정 (x,y좌표, 물체의 질량, 반경, 속도, 힘, 색상)
    this.pos = createVector(x, y);
    //좌표에 개체의 위치를 설정
    this.vel = p5.Vector.random2D();
    //개체의 속도를 random2D를 이용해 무작위로 부여
  ...
  cohesion(others) {
    ...
    others.forEach((each) => {
      ...
      if (each !== this) {
        ...
        //현재 개체의 위치와 주변 개체의 위치 사이의 거리 제곱 계산
        // 주변 개체의 좌표에서 현재 개체의 좌표를 빼고 각 차이를 제곱한 뒤 차이의 제곱을 합산)
        if (distSq < this.neighborhooodRad ** 2) {
          //거리 제곱이 주변 반경 제곱보다 작은지 확인
          steer.add(each.pos);
          //거리 제곱이 주변 반경 제곱보다 작을 시 개체의 위치를 steer 벡터에 추가
          cnt++; //cnt(count) 변수 증가
        }
      }
    });
    if (cnt > 0) {
      ...
      //주변 개체의 위치 계산
      steer.sub(this.pos);
      //steer 벡터에서 개체의 위치 뺌 (개체의 방향을 주변 개체 질량의 중심과 일치시킴)
      steer.setMag(this.speedMx);
      //steer 벡터의 크기를 개체의 최대 속도로 설정 (끌어당기는 힘이 최대 속도를 넘지 않도록 설정)
      steer.sub(this.vel);
      //steer 벡터에서 개체의 현제 속도를 뺌
      steer.limit(this.forceMx);
      //steer벡터의 크기를 끌어당기는 최대 힘으로 제한
    }
  ...
  applyForce(force) {
    ...
    // this.mass는 객체의 질량(mass)이며, 힘을 질량으로 나누어 가속도(acceleration)를 계산
    this.acc.add(forceDivedByMass);
    // forceDivedByMass 값을 가속도(acceleration)에 더한다. acc는 객체의 가속도를 나타낸다. 객체의 가속도는 객체에 작용하는 힘들을 고려하여 계산
  }
